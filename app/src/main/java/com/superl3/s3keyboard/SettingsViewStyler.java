...
final class SettingsViewStyler {
    ...
    static void button(Button button, Context context, boolean selected) {
        ...
        int iconTint = selected ? ui.selectedText : ui.controlText;
    ...
    static void iconButton(ImageButton button, Context context, int drawableResId) {
        if (button == null || context == null) {
            return;
        }
        SettingsUiPalette ui = SettingsUiPalette.from(context);
        button.setImageResource(drawableResId);
        button.setImageTintList(ColorStateList.valueOf(ui.controlText));
        button.setMinimumWidth(SettingsRowBuilder.dp(context, 48));
        button.setMinimumHeight(SettingsRowBuilder.dp(context, 48));
        button.setPadding(
                SettingsRowBuilder.dp(context, 11),
                SettingsRowBuilder.dp(context, 11),
                SettingsRowBuilder.dp(context, 11),
                SettingsRowBuilder.dp(context, 11));
        GradientDrawable background = new GradientDrawable();
